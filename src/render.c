...
static	void	complex_z(t_complex *z, int x, double ratio, t_mlx *data)
{
...
static void	handle_pixel(int x, int y, t_mlx *data)
{
	...
	while (i++ < data->frac.ite_def)
	{
		...
		{
			color = map(i, data->p[data->p1],
					data->p[data->p2], data->frac.ite_def);
			my_pixel_put(x, y, &data->img, color);
			return ;
		}
	}
	my_pixel_put(x, y, &data->img, data->p[data->p3]);
}
